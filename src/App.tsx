...
export default function App() {
  ...
  const [shoots, setShoots] = useState<Shoot[]>(() => {
    const saved = localStorage.getItem('photographer_shoots_v1');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (err) {
        console.error('Failed to parse saved shoots', err);
      }
    }
    return getMockShoots();
  });
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (list.length === 0 && !hasSeeded) {
        ...
        setShoots(mocks);
        localStorage.setItem('photographer_shoots_v1', JSON.stringify(mocks));
        ...
        setShoots(list);
        localStorage.setItem('photographer_shoots_v1', JSON.stringify(list));
      }
    }, (error) => {
